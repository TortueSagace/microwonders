...
### 3.1 - using IS[1]
Thanks to IS1's extended set of instructions and most noticeably its ability to shift registers of a constant amount as well as automatically branch when an overflow is detected, the multiplication can now be performed in linear time w.r.t. the operand's size as opposed to exponential time. Instead of decrementing one operand until it reaches zero while adding the other to the current value of the result's register, the following program performs conventional binary multiplication: it loops over every bit of r2, shifting the first operand of the correponding amount before adding it to the current value of the result's register.
...
The jar simulator execution trace on the left is compared with the online tool's on the right: for the 1.9_mul exercise with the signed configuration, an overflow is incorrectly detected when shifting left 0x7fff << 1 = 0xfffe. The online tool may be prone to errors that make its results diverge with those of the .jar simulator for the IS1 implementation of the RISC16.
